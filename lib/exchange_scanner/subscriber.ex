defmodule ExchangeScanner.Subscriber do
  ...
  if result.base == "BTC" do
    ...
    ExchangeScannerWeb.Endpoint.broadcast("room:public", "arbitrage_signal", %{result: result, count: count, date: date, summary: summary})    
    {:noreply, state}
  end
  ...
  def handle_info({:current_pair, result}, state) do
    ...
    ExchangeScannerWeb.Endpoint.broadcast("room:public", "current_pair", %{pair: pair, scan_count: scan_count})
